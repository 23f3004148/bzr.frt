...
- Frontend: https://bzzr.vercel.app  
- Backend/API/Socket: https://bzzrbackend-production.up.railway.app  
- On Vercel, set `VITE_API_BASE_URL` and `VITE_SOCKET_URL` to the backend URL (add Google/admin reset envs if used).  
- If env vars are missing on `bzzr.vercel.app`, the app falls back to the Railway backend automatically to avoid localhost calls.
...
2. **Configure environment:**
   - Root `.env.local` stores Vite client variables:
     - `VITE_API_BASE_URL=http://localhost:4000`
     - `VITE_SOCKET_URL=http://localhost:4000` (optional; defaults to API base)
...
- `/backend/src/controllers`: request/response adapters that call the service layer.  
- `/backend/src/services`: business logic shared across routes (auth, admin, config, etc.).  
- `/backend/src/repositories`: trusted Mongoose accessors so controllers/services stay focused on flow rather than queries.
...
- Socket errors: confirm `VITE_API_BASE_URL`/`VITE_SOCKET_URL` point to your backend and CORS is configured.
